...
public class Analizador {
    ...
    public Analizador(AnalizadorLexico l) throws IOException { //se cambio 1= l
        lex = l;
        mover();
    }

    void mover() throws IOException {
        busca = lex.explorar(); ////Se cambio escanear() por explorar()
    }
    ...
    Tipo tipo() throws IOException {
        ...
        if (busca.etiqueta != '[') {
            return p; // T −> basico // se caambio devuelbe =return
        } else {
    ...
    Instr instr() throws IOException {
        ...
        switch (busca.etiqueta) {
            ...
            case Etiqueta.WHILE:
                ...
                s1 = instr();
                nodowhile.inic(x, s1);
                Instr.Circundante = instrGuardada; // restablece Instr.Circundante return nodowhile;
            case Etiqueta.DO: // se cambio ;=:
                Do nododo = new Do();
                instrGuardada = Instr.Circundante;
                ...
                coincidir(';');
                nododo.inic(s1, x);
                Instr.Circundante = instrGuardada; // restablece Instr.Circundante return nododo;
            case Etiqueta.BREAK: // se cambio ;=:
    ...
    Acceso desplazamiento(Id a) throws IOException { // I −> [E] | [E] I 
        ...
        ubic = t1;
        while (busca.etiqueta == '[') { // multi−dimensional I −> [ E ] I coincidir(’[’);   i  =   bool(); 
            coincidir('[');
            tipo = ((Arreglo) tipo).de;
            w = new Constante(tipo.anchura);
            t1 = new Arit(new Token('*'), i, w);
